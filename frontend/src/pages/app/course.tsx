...
import CourseInformation from "../../types/CourseInformation";

export default function CoursePage() {
  const { courseID } = useParams();
  const [courseInformation, setCourseInformation] =
    useState<CourseInformation | null>(null);
    ...
    <div className="max-w-screen-3xl mx-auto">
      <div className="px-3 3xl:px-0 mt-3">
        <div className="py-6 rounded-t-xl bg-bg-300 grid place-content-center md:place-content-start relative mb-8">
          <img
            src={`${backendUrl}/api/course/image/${courseID}`}
            alt={courseID}
            className="md:ml-6 max-h-[120px]"
          />
        ...
        {courseInformation ? (
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {courseInformation.videos.map((video) => (
                  <Video
                    ...
                    image={`${backendUrl}/api/video/image/${video._id}`}
                    title={video.title}
                    course={{
          ...
          <div className="p-3">Lade Kurs...</div>
        )}
      </div>{" "}
    </div>
  );
